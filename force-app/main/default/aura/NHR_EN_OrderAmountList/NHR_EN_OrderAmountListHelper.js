({
	Sales: function(component, event, helper ,inTargetStr, inUnit) {
        var action = component.get("c.Call_EN_OrderAmountList");
        action.setParams({
    ...
    SalesOsaka: function(component, event, helper ,inTargetStr) {
        var action = component.get("c.Call_EN_OrderAmountListOsaka");
        action.setCallback(this, function(response) {
            var state = response.getState();
            var toastEvent = $A.get("e.force:showToast");
            if(state == "SUCCESS"){

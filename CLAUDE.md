...
- SOQLは直接書かず **`NHR_Dao`** のstaticメソッドに委譲する
...
### NHR_Dao（SOQL一元管理）
- 全SOQLは `NHR_Dao` のstaticメソッドとして定義
- 動的SOQLを文字列結合で構築するスタイル
- 定数は全て `NHR_Constant` から参照（ハードコード禁止）
...
- 定数は `NHR_Constant` に追加してから参照
- SOQLは `NHR_Dao` に追加してから呼び出す
...
- `NHR_Dao` を経由しないSOQLの直接記述（新規開発時）
- APIバージョン `66.0` 未満での新規メタデータ作成
- `NHR_Constant` を経由しない文字列のハードコード
...
「NHR_{モジュール}_{機能}Ctrl（Aura向け）を作成してください。
 取得条件: ○○
 SOQLは NHR_Dao に、定数は NHR_Constant に追加してください」
...
「NHR_{モジュール}_{機能}Ctrl の {メソッド名} に
 {条件} の処理を追加してください。
 NHR_Constant / NHR_Dao の変更も含めて対応してください」
```
